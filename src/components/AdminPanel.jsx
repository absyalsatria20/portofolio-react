...
const AdminPanel = ({ onProjectAdded }) => {
    ...
    // ==========================================
    const CLOUD_NAME = "vkyl7elo"; // Cloud name otomatis
    const UPLOAD_PRESET = "portofolio-cloudinary"; // Preset Unsigned milikmu

    const [isOpen, setIsOpen] = useState(false);
    const [uploadMode, setUploadMode] = useState('file'); // 'file' atau 'url'
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // JIKA MODE FILE: Upload dulu ke Cloudinary
            if (uploadMode === 'file') {
                if (!file) {
                ...
                // Endpoint beda untuk gambar dan video
                ...
                // Ambil link aman dari Cloudinary
                finalSrc = cloudResult.secure_url;
            }

            // SIMPAN DATA KE DATABASE (Backend)
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        ...
                        >
                            <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center bg-slate-100 hover:bg-rose-100 text-slate-500 hover:text-rose-600 rounded-full transition-colors z-10">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4 relative">
                                ...
                                {uploadMode === 'file' && (
                                    ...
                                    >
                                        {/* Input Hidden yang otomatis memanggil galeri di HP */}
                                ...
                                {uploadMode === 'url' && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" required={uploadMode === 'url'} placeholder="https://res.cloudinary.com/..." value={formData.src} onChange={(e) => setFormData({...formData, src: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-indigo-500"/>
                                        </div>
                                    </>
                                )}

                                {/* Thumbnail Time khusus Video */}
                                {formData.type === 'video' && (
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Thumbnail Time (Sec)</label>
                                        <input type="text" placeholder="Contoh: 1.5 (Tampil detik ke 1.5)" value={formData.thumbTime} onChange={(e) => setFormData({...formData, thumbTime: e.target.value})} className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-indigo-500"/>
                                    </motion.div>
                                )}
